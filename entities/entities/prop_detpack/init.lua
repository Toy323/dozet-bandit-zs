...
function ENT:OnTakeDamage(dmginfo)
	...
	if not self.Exploded and dmginfo:GetDamage() >= 9 then
		...
		if not (attacker:IsValid() and attacker:IsPlayer() and self:GetObjectOwner():IsPlayer() and attacker:Team() == self:GetObjectOwner():Team()) or not self:GetObjectOwner():IsPlayer() then
			self:Remove()
		end
	end
end
...
function ENT:Use(activator, caller)
	if self.Exploded or self:GetExplodeTime() ~= 0 or not activator:IsPlayer() or self:GetMaterial() ~= "" then return end
