...
if CLIENT then
	SWEP.TranslateName = "weapon_shovel_m_name"
	SWEP.TranslateDesc = "weapon_shovel_m_desc"
...
function SWEP:OnMeleeHit(hitent, hitflesh, tr)
	if tr.HitWorld and (tr.MatType == MAT_DIRT or tr.MatType == MAT_SNOW  or tr.MatType == MAT_SAND  or tr.MatType == MAT_GRASS) then
		local found = false
		local perc = math.random(3)
		if perc == 1 then
			if SERVER then
				
				for k,v in pairs(GAMEMODE.MethUsed) do
					if v and v:DistToSqr(tr.HitPos) <= 41800 and #GAMEMODE.MethUsed > 2 then
						found = true
						break
					end
				end
				if !found then 
					GAMEMODE.MethUsed[#GAMEMODE.MethUsed+1] = tr.HitPos
					...
					self:GetOwner():SendLua('surface.PlaySound("buttons/button8.wav")')
					return
				end
