function EFFECT:Init(data)
	...
	emitter:SetNearClip(40, 45)
		for i=1, math.random(12, 15) do
			local heading = VectorRand():GetNormalized()*power
			...
			particle:SetStartSize(12*power)
			particle:SetEndSize(4*power)
		...
		for i=1, math.random(5, 8) do
			...
			particle:SetVelocity(VectorRand():GetNormalized() *power * math.Rand(48, 82))
			particle:SetDieTime(math.Rand(2.2, 3.6))
			particle:SetStartAlpha(220)
			particle:SetEndAlpha(0)
			particle:SetStartSize(8*power)
			particle:SetEndSize(30*power)
		...
		for i=1, math.random(17, 21) do
			...
			local dir = VectorRand():GetNormalized() *power
			particle:SetVelocity(dir * math.Rand(500, 600) *power)
			particle:SetDieTime(math.Rand(1.0, 1.25))
			particle:SetStartAlpha(220)
			particle:SetEndAlpha(0)
			particle:SetStartSize(10*power)
			particle:SetEndSize(5*power)
			particle:SetRoll(math.Rand(0, 360))
			particle:SetRollDelta(math.Rand(-3, 3))
			particle:SetAirResistance(60)
			particle:SetGravity(dir * math.Rand(-100, -200))
		end
		for i=1, 2 do
			...
			particle:SetEndAlpha(0)
			particle:SetStartSize(30*power)
			particle:SetEndSize(20*power)
			particle:SetRoll(math.Rand(0, 360))
